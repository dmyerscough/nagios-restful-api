...
urlpatterns = patterns('',
    ...
    url('^notification/(?P<hostname>[-a-zA-Z0-9\.]+)/(?P<service>[-a-zA-Z0-9 \._]+)/$', Notifications.as_view()),

    # Schedule host or service checks
    url('^schedule/check/(?P<hostname>[-a-zA-Z0-9\.]+)/$', ScheduleChecks.as_view()),
    url('^schedule/check/(?P<hostname>[-a-zA-Z0-9\.]+)/(?P<service>[-a-zA-Z0-9 \._]+)/$', ScheduleChecks.as_view()),
